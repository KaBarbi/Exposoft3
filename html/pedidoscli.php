...
<body>
    ...
    // Verifica se o cliente está logado e obtém o ID do cliente
    session_start();
    if (isset($_SESSION['idcli'])) {
        $idcli = $_SESSION['idcli'];

        // Consulta SQL para selecionar apenas os pedidos do cliente logado
        $sql = "SELECT ped.idpedido, ped.idprod, cadprod.prodnome, ped.qtd, ped.totalped, ped.data_pedido, endercli.cidade, endercli.rua, endercli.numero, endercli.complemento
        FROM pedidos ped
        ...
        if ($qr->num_rows > 0) {
            ...
            echo "</table>";
            echo "</div>";
        } else { // caso nao tenha nenhum poedido aparece a mensagem abaixo
            echo "Nenhum pedido encontrado.";
        }

        $mysql->close();
    } else { // caso nao tenha nenhum cliente logado
